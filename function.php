...
function register()
{
    ...
    $query_user = "INSERT INTO user (role, nama, email, alamat, tgl_lahir, password) VALUES ('1','$nama', '$email', '$alamat', '$tgl_lahir','$password')";
...
function tambahSolusi()
{
    ...
    $deskripsi = htmlspecialchars($_POST['deskripsiSolusi']);
    $id_penyakit = htmlspecialchars($_POST['id_penyakit']);
    // query SQL diubah untuk memasukkan deskripsi
    $querySolusi = "INSERT INTO solusi VALUES ('', '$id_penyakit', '$solusi', '$deskripsi')";
...
function ubahSolusi($id_solusi)
{
    ...
    $deskripsi = htmlspecialchars($_POST['deskripsiSolusi']);
    // $penyakit = $_POST['id_penyakit'];
    $querySolusi = "UPDATE solusi SET solusi = '$solusi', id_penyakit = '$id_penyakit', deskripsi = '$deskripsi' WHERE id_solusi = '$id_solusi'";
    // $queryRelasi = "INSERT INTO relasi VALUES ('', '')"
    $exe = mysqli_query($koneksi, $querySolusi);
    if (!$exe) {
        die('Error pada database');
